...
/*数据解析依照自己需求来定，如果服务器直接返回百分比数据，则不需要客户端进行计算*/
     ...
     * @param array
     */
    public void parseMinutes1(JSONArray array) {
        int count = array.length();
        if (count == 0) {
            return;
        }
        JSONObject object1 = array.optJSONObject(0);
        baseValue = Float.parseFloat(object1.optString("YesterdayIncome"));

        for (int i = 0; i < count; i++) {

            JSONObject object = array.optJSONObject(i);
            MinutesBean minutesData = new MinutesBean();
            minutesData.time = object.optString("Min").substring(4);
            minutesData.cjprice = Float.parseFloat(object.optString("HQZJCJ"));

            if (i != 0) {
                JSONObject object2 = array.optJSONObject(i - 1);
                minutesData.cjnum = Float.parseFloat(object.optString("HBCJSL")) - Float.parseFloat(object2.optString("HBCJSL"));
            } else {
                minutesData.cjnum = Float.parseFloat(object.optString("HBCJSL"));
            }
            minutesData.total = Float.parseFloat(object.optString("Turnover"));
            minutesData.avprice = Float.parseFloat(object.optString("AveragePrice"));
            minutesData.cha = minutesData.cjprice - baseValue;
            minutesData.per = (minutesData.cha / baseValue);
            double cha = minutesData.cjprice - baseValue;
            if (Math.abs(cha) > permaxmin) {
    ...
    public void parseKLine1(JSONArray array) {
        int count = array.length();
        if (count == 0) {
            return;
        }
        ...
        for (int i = 0; i < count; i++) {
            KLineBean kLineData = new KLineBean();
            JSONObject object = array.optJSONObject(i);
            kLineData.date = object.optString("Time");
            kLineData.open = Float.parseFloat(object.optString("Opening"));
            kLineData.close = Float.parseFloat(object.optString("Closing"));
            kLineData.high = Float.parseFloat(object.optString("HighestPrice"));
            kLineData.low = Float.parseFloat(object.optString("Lowest"));
            kLineData.vol = Float.parseFloat(object.optString("Quantity"));
            volmax = Math.max(kLineData.vol, volmax);
            xValuesLabel.put(i, kLineData.date);
            kLineBeans.add(kLineData);
        }
